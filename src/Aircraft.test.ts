...
describe("Aircraft", () => {
  let scene: Scene;
  let mockInput: { pitch: number; roll: number; yaw: number; throttle: number; fire: boolean; cycleTarget: boolean; lockOn: boolean };

  beforeEach(() => {
    vi.resetAllMocks();
    scene = new (Scene as unknown as new () => Scene)();
    mockInput = { pitch: 0, roll: 0, yaw: 0, throttle: 0, fire: false, cycleTarget: false, lockOn: false };
  });
