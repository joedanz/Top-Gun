...
describe("WeaponSystem", () => {
  let scene: Scene;
  let mockAircraft: { mesh: { position: { x: number; y: number; z: number }; rotation: { x: number; y: number; z: number } }; input: { fire: boolean; cycleTarget: boolean; lockOn: boolean } };
  ...
  beforeEach(() => {
    ...
    mockAircraft = {
      ...
      input: { fire: false, cycleTarget: false, lockOn: false },
    };
  });
